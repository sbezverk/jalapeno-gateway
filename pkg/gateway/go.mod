module gateway

go 1.13

require (
	github.com/sbezverk/jalapeno=gateway/pkg/apis v0.0.0-00010101000000-000000000000
	github.com/sbezverk/jalapeno=gateway/pkg/dbclient v0.0.0-00010101000000-000000000000
	github.com/sbezverk/jalapeno=gateway/pkg/dbclient/mock v0.0.0-00010101000000-000000000000
	github.com/golang/glog v0.0.0-20160126235308-23def4e6c14b
	google.golang.org/grpc v1.26.0
)

replace (
	github.com/sbezverk/jalapeno=gateway/pkg/apis => ../apis
	github.com/sbezverk/jalapeno=gateway/pkg/dbclient => ../dbclient
	github.com/sbezverk/jalapeno=gateway/pkg/dbclient/mock => ../dbclient/mock
)
